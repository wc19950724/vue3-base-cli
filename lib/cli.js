...
var promptQuestions = async (name) => {
...
var deleteConfirm = async (dirName, dirPath) => {
  ...
  if (action) {
    ...
    await (0, import_rimraf.rimraf)(dirPath);
    return Promise.resolve();
  }
  return Promise.reject("Cancelled creation");
};
var installConfirm = async (dirName) => {
...
var selectCmd = async (needSelection = true) => {
  let command = "npm";
  const cmds = getCmds();
  if (cmds.length === 1) {
    command = cmds[0];
...
var cli = async () => {
  ...
  program.command("create <project-name>").option("-g, --git", "git clone url").action(async (projectName, options) => {
    try {
      ...
      await (0, import_rimraf2.rimraf)(gitFile);
